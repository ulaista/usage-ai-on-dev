...
func (s *Service) CreateIntent(ctx context.Context, title, goal string, constraints, affected, verification []string) (domain.Intent, error) {
	...
	in := domain.Intent{ID: id("INT"), Title: title, Goal: goal, Status: "active", Constraints: constraints, Affected: affected, Verification: verification, CreatedAt: now, UpdatedAt: now}
...
func (s *Service) Status(ctx context.Context) (map[string]any, error) {
	...
	economySummary, err := s.Store.EconomySummary(ctx)
	if err != nil {
		return nil, err
	}
	provider := "disabled"
	...
	status := map[string]any{
		...
		"telemetry":         telemetry,
		"token_economy":     economySummary,
	}
	if lifecycle, ok := s.Semantic.(interface{ Status() semantic.LifecycleStatus }); ok {
		status["semantic_lifecycle"] = lifecycle.Status()
	}
