...
func New(svc *core.Service) *mcp.Server {
	server:=mcp.NewServer(&mcp.Implementation{Name:"project-brain",Version:"v0.4.0"},nil)
	type emptyArgs struct{}
	mcp.AddTool(server,&mcp.Tool{Name:"brain_status",Description:"Return Project Brain status, active intent count, storage and telemetry summary."},func(ctx context.Context,_ *mcp.CallToolRequest,_ emptyArgs)(*mcp.CallToolResult,any,error){
		status,err:=svc.Status(ctx); if err!=nil{return nil,nil,err}; if view,err:=hardware.Review(ctx,svc.Config.StateDir);err==nil{status["hardware_policy"]=view}; return text(status)
	})
