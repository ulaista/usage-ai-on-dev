...
func addDeveloperFlowTools(server *mcp.Server, svc *core.Service) {
	...
	mcp.AddTool(server,&mcp.Tool{Name:"brain_project_begin",Description:"Capture a developer-session baseline before modification, including BASE and pre-existing USER_DIRTY file hashes."},func(ctx context.Context,_ *mcp.CallToolRequest,args flowArgs)(*mcp.CallToolResult,any,error){if args.SessionID==""||args.Task==""{return nil,nil,fmt.Errorf("session_id and task are required")};baseline,err:=(projectmode.Engine{Root:svc.Config.Root,StateDir:svc.Config.StateDir}).Begin(ctx,args.SessionID,args.Task);if err!=nil{return nil,nil,err};return text(baseline)})
