...
func semanticTerms(task string, limit int) []string {
	...
	for _, term := range strings.FieldsFunc(strings.ToLower(task), func(r rune) bool { return !(r>='a'&&r<='z'||r>='0'&&r<='9'||r>='а'&&r<='я') }) {
		if len([]rune(term))<3 || stop[term] || seen[term] { continue }
		seen[term]=true; out=append(out,term); if len(out)>=limit { break }
	}
	return out
}
