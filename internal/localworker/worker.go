...
const workerPromptVersion = "bounded-worker-v3-router"
...
func (w *Worker) Run(ctx context.Context, req Request) (Result, error) {
	...
	routeDecision, err := (router.Router{Stats:w.Store}).Decide(ctx, router.Request{Task:req.Task,TaskType:req.TaskType,Model:w.Model,ContextTokens:req.ContextTokens})
	if err != nil { return Result{}, err }
	...
	key:=economy.Fingerprint(workerPromptVersion,economy.NormalizeTask(req.Task),req.TaskType,w.Model,snapshot.Inventory.HardwareID,req.Context,strconv.Itoa(profile.HardContextTokens),strconv.Itoa(profile.MaxOutputTokens),routeDecision.Route)
...
func (w *Worker) callOllama(ctx context.Context,req Request,profile hardware.Profile)(workerResponse,error){
	...
	if req.ContextTokens>0 { needed:=req.ContextTokens+profile.MaxOutputTokens+512; if needed<2048{needed=2048}; if needed<numCtx{numCtx=needed} }
