...
func main(){
	...
	case "autotune":var models []string;if flag.NArg()>1&&strings.TrimSpace(flag.Arg(1))!=""{for _,name:=range strings.Split(flag.Arg(1),","){if v:=strings.TrimSpace(name);v!=""{models=append(models,v)}}};runner:=autotune.Runner{OllamaURL:cfg.OllamaURL,StateDir:cfg.StateDir};report,err:=runner.Run(ctx,autotune.Options{Models:models,MaxModels:4,OutputTokens:64});if err!=nil{log.Fatal(err)};if err:=autotune.SaveReport(autotune.ReportPath(cfg.StateDir),report);err!=nil{log.Fatal(err)};printJSON(map[string]any{"applied":false,"requires_user_acceptance":report.Recommendation.PreferredModel!="","report":report})
